...
public class DevRelQuotesApp extends DialogflowApp {
  ...
  private static final String CONTENT_URL = "https://raw.githubusercontent.com/actions-on-google/dialogflow-quotes-java/master/quotes.json";
  private static final String BACKGROUND_IMAGE = "https://lh3.googleusercontent.com/t53m5nzjMl2B_9Qhwc81tuwyA2dBEc7WqKPlzZJ9syPUkt9VR8lu4Kq8heMjJevW3GVv9ekRWntyqXIBKEhc5i7v-SRrTan_=s688";

  private static final Logger LOGGER = LoggerFactory.getLogger(DevRelQuotesApp.class);
  ...
  public CompletableFuture<ActionResponse> defaultWelcomeIntent(ActionRequest request) {
    ...
    try {
      ...
      LOGGER.error("Exception while fetching and parsing data from " + CONTENT_URL + ":", ex);

      // Handle error gracefully by sending a response
      responseBuilder.add(rb.getString("problem"))
          .endConversation();
    ...
    responseBuilder.add(new SimpleResponse()
        ...
        .setTextToSpeech(String.format(
            rb.getString("long_attribution"),
            quote.authorName,
            quote.quoteText)));
    ...
    if (request.hasCapability("actions.capability.SCREEN_OUTPUT")) {
      responseBuilder.add(new BasicCard()
          .setTitle(String.format(rb.getString("short_attribution"), quote.authorName))
          .setFormattedText(quote.quoteText)
          .setImage(new Image()
              .setUrl(BACKGROUND_IMAGE)
              .setAccessibilityText(rb.getString("accessibility_text"))));
  ...
  private static QuoteDetails parseQuotesJSON(JsonElement responseRoot) {
    ...
    String quote = quotes.get((int) (Math.random() * quotes.size())).getAsString();
  ...
  private static class QuoteDetails {
    ...
    private QuoteDetails(String information, String authorName, String quoteText) {
